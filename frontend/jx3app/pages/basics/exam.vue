				...
				<swiper-item v-for="(subject,index) in subjectList">
					...
					<view v-if="index-subjectIndex>=-1&&index-subjectIndex<=1">
						...
						<view>
							...
							<radio-group class="block" @change="RadioboxChange" v-if="subject.type===1||subject.type===2">F
							...
							<checkbox-group class="block" @change="CheckboxChange" v-else-if="subject.type===3">
								<view class="cu-form-group" v-for="option in subject.optionList">
									...
									<view class="title  text-black animation-reverse margin-sm basis-sm shadow" data-class="fades" :class="['bg-red',animation=='fades'?'animation-fade' :'']" @tap="Toggle">{{option.id}}.{{option.content}}</view>
								</view>
							</checkbox-group>
